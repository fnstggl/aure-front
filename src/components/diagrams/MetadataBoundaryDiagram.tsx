...
function MiniNode({ label, sub, tone }: { label: string; sub: string; tone: "neutral" | "amber" }) {
  return (
    <div
      className={cn(
        ...
        <div className="flex-1 rounded-md border border-signal/20 bg-signal/[0.03] p-4">
          ...
          <div className="flex flex-col items-stretch gap-0">
            {AURELIUS_NODES.map((n, i) => (
              <div key={n.label}>
                <MiniNode {...n} tone="amber" />
                {i < AURELIUS_NODES.length - 1 && <RailY flowing={inView} className="mx-auto h-2.5" />}
              </div>
            ))}
          </div>
        </div>
