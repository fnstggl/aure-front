...
| Ablation: single-surface -4.47%, SLA gate failed, 173.5% value forfeited; physics-guided +100.48%, 30.3% regret; coupled search +161.31%, 0% measured regret; ~40 evaluations per decision | `data/external/mpc_controller/physics_guided_planner_backtest.json`; `research/PHYSICS_GUIDED_PLANNER_RESULTS.md` L33-41. | Fenced at both ends of page 2 so it cannot be mistaken for the headline: a line above the columns ("Mechanism experiment, frozen continuity harness. The headline benchmark is separate and larger: pages 1 and 4.") and a closing bridge stating the 8.24x headline is the same coupled search run separately at full uncapped load against the production-class composite, different baseline and load, never mixed. The "same mechanism drives the uncapped result" bridge is supported by the published report, Appendix B. |
