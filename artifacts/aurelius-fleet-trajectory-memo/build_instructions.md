...
- **No node squares in the line figures.** In FIG 02/03, trajectories and spines
  are clean lines only; no decorative squares. (FIG 01 is the deliberate
  exception: it *is* a field of squares, a data plate rather than a line diagram,
  so its cells are not "nodes" and the line grammar below does not apply to it.
  It still shares the coordinate system, the grayscale palette, and the plate
  chrome, so it reads as one system with the rest.) Where a line figure needs a
  boundary, use a dashed vertical `#111110` 0.75pt line (`stroke-dasharray="2 3"`).
...
result + validation. Page 1 leads with the FIG 01 decision-space field and a
...
### Per figure
- **FIG 01** (page 1): **the space of one decision**, a light-mode adaptation of
  the site's world-model graphic. One large field of small squares stands in for
  the ~4.7M representable coupled-policy configurations per decision
  (connected-surface product, `theoretical_combinations()` = 4,723,920, the hero
  number), drawn in quiet warm grays. A tilted bloom of the **72-75** the bounded
  search actually scores (PJM 75 / ERCOT 72 / CAISO 72, `candidate_bundles_evaluated`)
  is one step darker (outer `#8C8C8C`, inner finalists `#6E6A62`), converging on
  the **1** returned configuration: a single ink `#111110` cell with four
  registration ticks. Restrained, one field, one strong quantitative statement,
  almost no labels; it reads as an artifact, not a flowchart. The field is
  deterministic and generated by `source/fig01_field.py` (a sine hash, stable
  across rebuilds); run that script to regenerate the inline SVG after changing
  any field parameter. The plate-cap runs smaller and tighter than
  the default (`.fig01-cap`) so two clauses fit on one line: it scopes bounded
  search to the representable space and carries the field legend ("each square
  represents one multi-period coupled-policy configuration"). The per-market
  scored counts (PJM 75 / ERCOT 72 / CAISO 72) are ledger-only. The page-1 lead
  scopes the zero-regret claim to separately tractable spaces.
...
- Because the SVG viewBox width (~520) now matches the content column, figures
  ...
  152, FIG 03a 84, FIG 03b 56) or pages 2 and 3 overflow their
  footers.
